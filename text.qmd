...
sum2mat <- function(expr_str){
  ...
  sum2mat_convert <- function(expr,sum_var=NULL){
    ...
    if(op == "s"){
      ...
      if(1){
        summation_var <- expr[[3]][[2]]
        daiichi <- expr[[2]][-1]
        Mat_symbol_vec <- daiichi %>% sapply(function(x)x[[2]])
        subv_mat <- daiichi %>% sapply(function(x)x[-(1:2)] %>% as.character)
        # subv <- subv_vec[,2]; Mat_symbol <- Mat_symbol_vec[2]
        
        result <- element2Matrix_2(subv_mat,Mat_symbol_vec,summation_var)
        return(result)
      }else{
        print("想定外です１")
        return(deparse(expr))
...
element2Matrix_2 <- function(subv_mat, Mat_symbol_vec, summation_var=NULL, operator = NULL){
    
    if(is.null(summation_var)){
      if(0){
        # no summationの場合。まだ、全体には未実装。
        summation_var = NULL
        Mat_symbol_vec <- list("a", "b")
        subv_mat <- matrix(c("i","i", "i", "j"),2)
        subv_mat <- matrix(c("i","j", "i", "j"),2)
        subv_mat <- matrix(c("j","i", "i", "j"),2)
        operator = "*"
        operator = "+"
        operator = "-"
      }
    ...
    sub_logical <- (subv_mat == summation_var)
...
expr_str <- "s(a[i,i]*b[i,j],{i})"; sum2mat(expr_str)
